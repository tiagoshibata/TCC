            ...
            for next_scene in scene_frames[1:]:
                if len(scene[1]) < 3 or len(next_scene[1]) < 3:
                    for frame in [*scene[0].iterdir(), *next_scene[0].iterdir()]:
                        shutil.copy(frame, temporary_directory / frame.name)
                else:
                    scene = next_scene
                    frames = sorted(temporary_directory.iterdir())
                    if frames:
                        if merge_prompt(temporary_directory, viewer=viewer):
                            for frame in temporary_directory.iterdir():
                                shutil.copy(frame, dataset.get_scene_directory(movie_directory, frames[0].stem) / frame.name)
                                frame_scene = movie_directory / frame.stem
                                if frame_scene.exists():  # if this frame is at the start of a scene
                                    shutil.rmtree(frame_scene)
                        for frame in temporary_directory.iterdir():
                            frame.unlink()
