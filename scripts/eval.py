...
def parse_args():
    ...
    parser.add_argument('dataset', type=directory_path, help='dataset')
...
def main(args):
    ...
    for scene, frames in scenes.items():
        ...
        ground_truth_state = state = np.dstack(read_image_lab(scene, frames[0]))
        for i, frame in enumerate(frames[1:]):
            l, ab = read_image_lab(scene, frame)

            # output = predict(ground_truth_state, l)
            # write_image_lab('output_ground_truth_state_{}_{}.png'.format(scene_number, i), l, output[0])
            # ground_truth_state = np.dstack((l, ab))

            output = predict(state, l)
            write_image_lab('output_{:04d}_{:04d}.png'.format(scene_number, i), l, output[0])
            state = np.dstack((l, output[0]))
            # np.save('output_{}_{}.npy'.format(scene_number, i), state)
